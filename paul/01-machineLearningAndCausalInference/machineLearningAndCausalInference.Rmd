...
parameters that results in a better estimator. Another important
caveat is that I did use sample splitting. @chernozhukov2018 use
sample splitting to control the terms 
$$
\begin{align*}
b = & \En[(m(x_i)-\hat{m}(x_i))\epsilon_i] \\
c = & \En[v_i(\mu(x_i) - \hat{\mu}(x_i))] 
\end{align*}
$$
These might also be the source of the problem with the random forest
estimates. 
...
networks approximate this class of functions well, and compares his
result to other function approximation results. 
</div>
