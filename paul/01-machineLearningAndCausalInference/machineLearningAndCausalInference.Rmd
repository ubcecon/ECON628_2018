...
\begin{align*}
  ...
  \theta = & \Er\left[d_i \frac{y_i - \Er[y_i | d_i = 1,
...
$$
\theta = \frac{\Er\left[\Er[y|z=1,x] - \Er[y|z=0,x]\right]}
{\Er\left[\Er[d|z=1,x]-\Er[d|z=0,x]\right]}
$$
...
- $0 = \En[d_i(y_i - \tilde{\theta} d_i - \hat{f}(x_i)) ]$
...
```{r, pipelineErrorPlot, cache=TRUE, echo=FALSE}
...
$$
    ...
    - Small $\gamma \to 0$ with $n$, and $\log(1/\gamma) \lesssim \log(p \vee n)$
